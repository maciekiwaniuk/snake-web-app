...
To enable e-mail service and send e-mails properly you need to fill all necessary variables related with MAIL_ in .env and after that set MAIL_SERVICE_ENABLED to true.
...
Redis is necessary to count for example total visits on website. To enable redis service you need to fill all crucial variables related with REDIS_ in .env and after that set REDIS_CONFIGURED to true.

    REDIS_CONFIGURED=true

# Application usage

Every account generated by db:seed command except with administrative permissions has same password:
